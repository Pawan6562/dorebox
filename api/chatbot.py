...
# --- IMPORTANT CHANGE 1: Use Environment Variable (BEST PRACTICE) ---
# Vercel mein settings ke andar 'OPENROUTER_API_KEY' set karein.
# Code ab automatically us key ko fetch karega.
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")
MODEL_NAME = "nvidia/nemotron-nano-12b-v2-vl:free"

# Vercel/Python mein function ka naam 'handler' ya 'vercel_handler' ho sakta hai,
# yeh Vercel ki configuration par depend karta hai.
def handler(request):
    print("DEBUG: Handler function started.")
    ...
    print("DEBUG: API Key is present. Starting request parsing.")

    try:
        # --- Parse Request Body ---
        try:
            # Vercel mein 'request.body' ek byte string hota hai, jisko decode karna zaroori hai.
        ...
        if not openrouter_response.ok:
            # OpenRouter se error aane par uska status code forward karna
        ...
        print("DEBUG: Successfully received response from OpenRouter.")
        return create_response(200, data)

    except requests.exceptions.RequestException as e:
        print(f"Network Error contacting OpenRouter: {e}")
        return create_response(503, {
            "error": "Failed to connect to the external AI service (Network issue).", 
            "details": str(e),
            "status": 503
        })
